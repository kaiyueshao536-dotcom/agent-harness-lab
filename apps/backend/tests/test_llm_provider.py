...
def test_loads_qwen_configuration_from_an_explicit_file(tmp_path: Path) -> None:
...
def test_provider_constructs_model_with_config(tmp_path: Path) -> None:
...
def test_provider_constructs_embedding_model_with_config(tmp_path: Path) -> None:
    config = load_llm_provider_config(
        config_path=_write_config(tmp_path, api_key="test-secret")
    )
...
@pytest.mark.asyncio
async def test_readiness_succeeds_without_exposing_secret(tmp_path: Path) -> None:
    config = load_llm_provider_config(
        config_path=_write_config(tmp_path, api_key="test-secret")
    )
...
@pytest.mark.asyncio
async def test_readiness_failure_is_safe(tmp_path: Path) -> None:
    config = load_llm_provider_config(
        config_path=_write_config(tmp_path, api_key="test-secret")
    )
    provider = QwenOpenAIProvider(
